...
class WavesManager {
  ...
  async start() {
    while (this.looping) {
      ...
      for (let i = 0; i < wave.length; ++i) {
        ...
        for (let j = 0; j < quantity; ++j) {
          ...
          for (let k = 0; k < map.enemyBases.length; ++k) {
            ...
            enemyManager.add(
              this.enemyFactory(enemyClass, enemySpecsMultiplier, base)
            );
          }
      ...
      if (!this.looping) break;
      await asyncSleepIntervalSecond(
        this.delayBetweenWaves,
        interfaceManager.setWaveDelay.bind(interfaceManager)
      );
  ...
  enemyFactory<T extends Enemy | AnimatedEnemy, K extends keyof T>(
    enemyClass:
      | { new (base: Base): T }
      | { new (base: Base, numberOfFrames: number, ticksPerFrame: number): T },
    enemySpecsMultiplier: { [k: string]: any } | undefined,
    base: Base
  ): Enemy | AnimatedEnemy {
    const enemy = new enemyClass(base, 0, 0);

    if (enemySpecsMultiplier) {
      for (const [specKey, specMultiplier] of Object.entries(
